-- Fix the ambiguous column reference issue and initialize sample Lode Mien Bac data

-- Drop existing function if exists
DROP FUNCTION IF EXISTS initialize_lode_mien_bac_sample_data();
...
BEGIN
    ...
    FOR i IN 1..array_length(session_dates, 1) LOOP
        ...
        -- Check if session already exists (use table alias to avoid ambiguity)
        SELECT gs.id INTO existing_session_id
        FROM public.game_sessions gs
        WHERE gs.game_type = game_type_val 
