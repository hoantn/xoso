-- Initialize sample Lode Nhanh data for both 1-minute and 3-minute games
...
BEGIN
    ...
    session_time := DATE_TRUNC('minute', current_time);
    session_time := session_time + INTERVAL '3 minutes' - ((EXTRACT(MINUTE FROM session_time)::INT % 3) || ' minutes')::INTERVAL;
    session_number_val := (
        EXTRACT(YEAR FROM session_time)::TEXT ||
        LPAD(EXTRACT(MONTH FROM session_time)::TEXT, 2, '0') ||
        LPAD(EXTRACT(DAY FROM session_time)::TEXT, 2, '0') ||
