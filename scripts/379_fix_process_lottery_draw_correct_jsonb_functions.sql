...
-- Create corrected function using proper PostgreSQL JSONB functions
        ...
        IF v_bet_record.bet_type LIKE '%lo%' THEN
            ...
            FOREACH v_number IN ARRAY v_bet_numbers_str LOOP
                ...
                IF v_count > 0 THEN
                    ...
                    v_hit_counts := v_hit_counts || jsonb_build_object(v_number, v_count);
                END IF;
            END LOOP;

            -- Calculate total hits for all numbers using proper JSONB iteration
            v_hit_count := 0;
            FOR v_key IN SELECT jsonb_object_keys(v_hit_counts) LOOP
                v_hit_count := v_hit_count + (v_hit_counts->>v_key)::INTEGER;
            END LOOP;
...
DO $$
BEGIN
    RAISE NOTICE '=== PROCESS LOTTERY DRAW FUNCTION FIXED WITH PROPER JSONB FUNCTIONS ===';
    RAISE NOTICE 'Fixed issues:';
    RAISE NOTICE '  ✅ Removed non-existent jsonb_object_keys_length() function';
    RAISE NOTICE '  ✅ Using proper jsonb_object_keys() iteration';
    RAISE NOTICE '  ✅ Using correct database schema (results_data, winning_numbers)';
    RAISE NOTICE '  ✅ Proper JSONB object manipulation';
    RAISE NOTICE 'Function ready for lottery draw processing.';
END $$;
