-- Create Lode Nhanh sample data with simple session_number format
-- Using YYYYMMDD + sequence format like existing lode_mien_bac data
...
DECLARE
    ...
    -- Base session number (YYYYMMDD format like existing data)
    base_session_number INTEGER;
    
BEGIN
    -- Get base session number from current date (YYYYMMDD format)
    base_session_number := (EXTRACT(YEAR FROM base_time)::TEXT || 
                           LPAD(EXTRACT(MONTH FROM base_time)::TEXT, 2, '0') || 
                           LPAD(EXTRACT(DAY FROM base_time)::TEXT, 2, '0'))::INTEGER;
    ...
    FOR i IN 1..20 LOOP
        ...
        -- Generate session number: base + sequence (1001, 1002, 1003, etc.)
        session_number_val := base_session_number + 1000 + i;
    ...
    FOR i IN 1..20 LOOP
        ...
        -- Generate session number: base + sequence (2001, 2002, 2003, etc.)
        session_number_val := base_session_number + 2000 + i;
    ...
    -- Open 1p session
    session_number_val := base_session_number + 1100;
    ...
    -- Open 3p session
    session_number_val := base_session_number + 2100;
...
SELECT 
    ...
    CASE 
        WHEN results_data->'bonus_info'->>'special_round' = 'true' THEN 'x2'
        ELSE ''
    END as bonus_indicator
FROM public.game_sessions 
WHERE game_type IN ('lode_nhanh_1p', 'lode_nhanh_3p')
