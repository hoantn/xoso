-- ✅ SỬA LỖI: UUID vs TEXT comparison
-- Sửa lỗi type mismatch trong transaction duplicate check
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            IF is_winner AND win_amount > 0 THEN
                ...
                -- ✅ SỬA LỖI: Kiểm tra transaction trùng lặp với đúng kiểu dữ liệu
                SELECT COUNT(*) INTO existing_transaction_count
                FROM transactions 
                WHERE reference_id = bet_record.id  -- ✅ UUID = UUID (không cast)
                AND type = 'game_win';
                ...
                IF existing_transaction_count = 0 THEN
                    ...
                    INSERT INTO transactions (
                        ...
                        'completed',
                        bet_record.id,  -- ✅ UUID reference_id
                        jsonb_build_object(
    ...
    RAISE NOTICE '📊 BÁO CÁO CUỐI CÙNG:';
    
    SELECT 
        'PHIÊN 2336 - TỔNG KẾT' as summary_type,
        COUNT(*) as total_bets,
        COUNT(*) FILTER (WHERE ub.status = 'won') as won_bets,
        COUNT(*) FILTER (WHERE ub.status = 'lost') as lost_bets,
        COUNT(*) FILTER (WHERE ub.status = 'pending') as pending_bets,
...
SELECT 
    '🏆 KẾT QUẢ CUỐI CÙNG PHIÊN 2336' as title,
    COUNT(*) as total_bets,
    COUNT(*) FILTER (WHERE ub.status = 'won') as won_bets,
    COUNT(*) FILTER (WHERE ub.status = 'lost') as lost_bets,
...
-- Kiểm tra user balance sau khi xử lý
SELECT 
