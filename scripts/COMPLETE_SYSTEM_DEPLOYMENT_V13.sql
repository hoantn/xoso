-- =======================================================
-- MASTER SCRIPT KHÔI PHỤC HỆ THỐNG DATABASE V13
-- Bao gồm tất cả tính năng từ V12 + Withdrawal Feature
...
-- 2. XÓA CÁC TRIGGER VÀ FUNCTION CÓ VẤN ĐỀ
...
-- 3. XÓA TẤT CẢ CÁC FUNCTION CŨ ĐỂ TẠO LẠI
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS purchase_proxy_plan(UUID, UUID) CASCADE;

-- 4. TẠO BẢNG USERS NẾU CHƯA TỒN TẠI
...
-- 5. THÊM CÁC CỘT THIẾU VÀO BẢNG USERS
DO $$
BEGIN
...
-- 6. TẮT ROW LEVEL SECURITY TRÊN BẢNG USERS
ALTER TABLE users DISABLE ROW LEVEL SECURITY;

-- 7. XÓA TẤT CẢ POLICIES TRÊN BẢNG USERS
DO $$
DECLARE
...
-- 8. TẠO BẢNG PROXIES NẾU CHƯA TỒN TẠI
...
-- 9. THÊM CÁC CỘT THIẾU VÀO BẢNG PROXIES
DO $$
BEGIN
...
-- 10. TẮT ROW LEVEL SECURITY TRÊN BẢNG PROXIES
ALTER TABLE proxies DISABLE ROW LEVEL SECURITY;

-- 11. TẠO BẢNG PROXY_PLANS
...
-- 12. THÊM CÁC CỘT THIẾU VÀO BẢNG PROXY_PLANS
DO $$
BEGIN
...
-- 14. THÊM CÁC CỘT THIẾU VÀO BẢNG PROXY_ORDERS
DO $$
BEGIN
...
-- 16. THÊM CÁC CỘT THIẾU VÀO BẢNG TRANSACTIONS
DO $$
BEGIN
...
-- 17. CẬP NHẬT CONSTRAINT CHO TRANSACTIONS TYPE (bao gồm withdrawal)
...
-- 19. TẠO BẢNG WITHDRAWAL_REQUESTS (MỚI)
...
-- 21. TẮT ROW LEVEL SECURITY CHO TẤT CẢ BẢNG
ALTER TABLE users DISABLE ROW LEVEL SECURITY;
ALTER TABLE proxies DISABLE ROW LEVEL SECURITY;
...
-- 22. TẠO FUNCTION UPDATE_UPDATED_AT_COLUMN
...
-- 23. TẠO TRIGGERS CHO UPDATE_UPDATED_AT_COLUMN
...
-- 25. TẠO FUNCTION PURCHASE_PROXY_PLAN
...
-- 26. CẬP NHẬT GIÁ TRỊ MẶC ĐỊNH CHO CÁC CỘT NULL
...
-- 27. THÊM ADMIN ACCOUNT NẾU CHƯA CÓ
...
-- 28. THÊM PROXY PLANS MẪU NẾU CHƯA CÓ
...
SELECT 'Hệ thống database V13 đã được cấu hình thành công!' AS message;
SELECT 'Username: admin / Password: admin123' AS admin_account;
SELECT 'Đã bao gồm tính năng Withdrawal Request' AS new_feature;
