-- Create Lode Nhanh sample data with correct session_number format
-- Using YYMMDDHHMM format to stay within INTEGER range
...
DECLARE
    ...
    -- Session number format variables
    year_part TEXT;
    month_part TEXT;
    day_part TEXT;
    hour_part TEXT;
...
BEGIN
    ...
    FOR i IN 1..20 LOOP
        ...
        -- Generate session number as YYMMDDHHMM (10 digits) to fit in INTEGER
        year_part := RIGHT(EXTRACT(YEAR FROM session_time)::TEXT, 2);
        month_part := LPAD(EXTRACT(MONTH FROM session_time)::TEXT, 2, '0');
        day_part := LPAD(EXTRACT(DAY FROM session_time)::TEXT, 2, '0');
        hour_part := LPAD(EXTRACT(HOUR FROM session_time)::TEXT, 2, '0');
        minute_part := LPAD(EXTRACT(MINUTE FROM session_time)::TEXT, 2, '0');
        
        session_number_val := (year_part || month_part || day_part || hour_part || minute_part)::INTEGER;
    ...
    FOR i IN 1..20 LOOP
        ...
        day_part := LPAD(EXTRACT(DAY FROM session_time)::TEXT, 2, '0');
        hour_part := LPAD(EXTRACT(HOUR FROM session_time)::TEXT, 2, '0');
        minute_part := LPAD((EXTRACT(MINUTE FROM session_time)::INT / 3 * 3)::TEXT, 2, '0');
    ...
    day_part := LPAD(EXTRACT(DAY FROM session_time)::TEXT, 2, '0');
    hour_part := LPAD(EXTRACT(HOUR FROM session_time)::TEXT, 2, '0');
    minute_part := LPAD(((EXTRACT(MINUTE FROM session_time)::INT / 3 + 1) * 3)::TEXT, 2, '0');
