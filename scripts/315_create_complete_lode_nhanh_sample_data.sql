...
DECLARE
    ...
    FUNCTION generate_5_digit() RETURNS TEXT AS $inner$
    BEGIN
    ...
    FUNCTION generate_4_digit() RETURNS TEXT AS $inner$
    BEGIN
    ...
    FUNCTION generate_3_digit() RETURNS TEXT AS $inner$
    BEGIN
        RETURN LPAD((RANDOM() * 999)::INT::TEXT, 3, '0');
    END;
    $inner$ LANGUAGE plpgsql;
    
    -- Helper function to generate random 2-digit number
    FUNCTION generate_2_digit() RETURNS TEXT AS $inner$
    BEGIN
...
-- Show sample results with complete prize structure
SELECT 
    game_type,
    session_number,
    status,
    ...
    results_data->>'first_prize' as first_prize,
    jsonb_array_length(results_data->'second_prize') as second_prize_count,
    jsonb_array_length(results_data->'third_prize') as third_prize_count,
    jsonb_array_length(results_data->'fourth_prize') as fourth_prize_count,
    jsonb_array_length(results_data->'fifth_prize') as fifth_prize_count,
    jsonb_array_length(results_data->'sixth_prize') as sixth_prize_count,
    jsonb_array_length(results_data->'seventh_prize') as seventh_prize_count,
...
-- Show detailed prize structure for latest completed sessions
SELECT 
    game_type,
    session_number,
    results_data->>'special_prize' as "Giải ĐB",
    results_data->>'first_prize' as "Giải Nhất", 
    results_data->'second_prize' as "Giải Nhì (2 số)",
    results_data->'third_prize' as "Giải Ba (6 số)",
    results_data->'fourth_prize' as "Giải Tư (4 số)",
    results_data->'fifth_prize' as "Giải Năm (6 số)",
    results_data->'sixth_prize' as "Giải Sáu (3 số)",
    results_data->'seventh_prize' as "Giải Bảy (4 số)"
FROM public.game_sessions 
WHERE game_type IN ('lode_nhanh_1p', 'lode_nhanh_3p')
